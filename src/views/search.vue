<template>
  <div class="search">
    ...
    <div>
      <div class="hot-keys"  v-show="hotKeys.length && !query">
        <h1 class="title">热门搜索</h1>
        <ul>
          <li class="item" v-for="item in hotKeys" :key="item.id" @click="addQuery(item.key)"><span>{{item.key}}</span></li>
        </ul>
      </div>
      <div class="search-history" v-show="searchHistory.length">
        ...
        <confirm ref="confirmRef" text="是否清空所有搜索历史" confirmBtnText="清空" @confirm="clearSearch"></confirm>
    ...
    <router-view v-slot="{ Component }">
      <transition appear name="slide">
        <component :data="selectedSinger" :is="Component"></component>
      </transition>
    </router-view>
  </div>
</template>
