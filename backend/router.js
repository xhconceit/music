  ...
  app.get('/api/search', (req, res) => {
    ...
    get(url, data).then((response) => {
      ...
      if (data.code === ERR_OK) {
        ...
        let songList = []
        if (data.data.song && data.data.song.itemlist.length) {
          songList = data.data.song.itemlist
        }
        let singer = []
        if (data.data.singer && data.data.singer.itemlist.length) {
          singer = data.data.singer.itemlist
        }
        const hasMore = 0
        res.json({
          code: ERR_OK,
